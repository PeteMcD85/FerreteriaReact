...
no_payment_orders =  Order.all.select(:id).where('COALESCE(credit_card_payed,0) + COALESCE(debit_payed,0)+ COALESCE(cash_payed,0)+ COALESCE(check_payed,0)<> total').distinct
...
#today =  todays date in a certain format ???
today = Date.today.strftime("%F")

#payment greater than Total
payments_greater_total = Order.all.where('(cash_payed + credit_card_payed + debit_payed + check_payed) <> total ')
...
# Creates a file for all relevant Models
